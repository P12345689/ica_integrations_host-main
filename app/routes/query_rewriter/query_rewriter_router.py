# -*- coding: utf-8 -*-
"""
Author: Mihai Criveti, modified by Matt Colman
Description: Generates engineered prompts from provided description

This module provides an API endpoint for generating high quality prompts based on a user's prompt.
...
Examples:
    ...
    >>> client = TestClient()
    >>> response = client.post("/query_rewriter/invoke", json={"input": "A helpful assistant to generate engineered prompts"})
...
template_env = Environment(loader=FileSystemLoader("app/routes/query_rewriter/templates"))
log.debug("Jinja2 environment initialized with template directory.")


class UserPrompt(BaseModel):
    """
    Represents the user's prompt for generating an engineered prompt.

    Attributes:
        input (str): The description of the prompt to be generated.
    """
...
def add_custom_routes(app: FastAPI) -> None:
    ...
    @app.post("/query_rewriter/invoke")
    async def query_rewriter(request: Request) -> JSONResponse:
        """
        Endpoint for generating engineered prompts based on a user's prompt.

        Args:
            request (Request): The incoming request object.

        Returns:
            JSONResponse: The response containing the generated prompt and examples.
        ...
        try:
            ...
            result = prompt_template.render(response={"prompt": task_response, "examples": examples_response})
